...
// TEMPORARY - verification of the Inline SDK script-URL fix. Not called
// from anywhere, not reachable from the browser. Remove after the test.
export const seedInlineFixTestEvent = internalMutation({
  args: {},
  handler: async (ctx): Promise<{ eventId: string; ticketTypeId: string; slug: string }> => {
    const now = Date.now();
    const slug = `inline-fix-test-${now}`;
    const eventId = await ctx.db.insert("events", {
      title: "Inline Fix Verification",
      description: "Temporary event for testing the checkout Inline SDK fix.",
      venue: "N/A",
      address: "N/A",
      city: "Accra",
      ...
      status: "published",
      slug,
      organizerName: "Verification Test",
      createdAt: now,
    });
    ...
    return { eventId, ticketTypeId, slug };
  },
});

// TEMPORARY - remove alongside seedInlineFixTestEvent.
export const deleteInlineFixTestData = internalMutation({
