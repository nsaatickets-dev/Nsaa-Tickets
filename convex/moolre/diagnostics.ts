...
// TEMPORARY - verification of the reusable-field type fix. Not called
// from anywhere, not reachable from the browser. Remove after the test.
export const seedReusableFixTestEvent = internalMutation({
  args: {},
  handler: async (ctx): Promise<{ eventId: string; ticketTypeId: string }> => {
    const now = Date.now();
    const eventId = await ctx.db.insert("events", {
      title: "Checkout Fix Verification",
      description: "Temporary event for testing the reusable-field fix.",
      venue: "N/A",
      address: "N/A",
      city: "Accra",
...
// TEMPORARY - remove alongside seedReusableFixTestEvent.
export const deleteReusableFixTestData = internalMutation({
